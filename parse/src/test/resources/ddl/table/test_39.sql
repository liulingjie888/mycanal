...
RENAME
TABLE `tb_imepdzhsgh` TO `tb_rawvvsgmse`;
RENAME
TABLE `tb_lkcruokdfh` TO `tb_scytcshwjf`, `tb_rawvvsgmse` TO `tb_utzdjdjsln`;
RENAME
TABLE `tb_scytcshwjf` TO `tb_hvqlhrcjcf`, `tb_utzdjdjsln` TO `tb_wscheecoug`;
RENAME
TABLE `tb_wscheecoug` TO `tb_ouvrqoxnvh`, `tb_hvqlhrcjcf` TO `tb_pactwzqamv`;
RENAME
TABLE `tb_eydxucudno` TO `tb_ebnesypzau`, `tb_ouvrqoxnvh` TO `tb_onrsvwgpqb`;
RENAME
TABLE `tb_ebnesypzau` TO `tb_xfnqchameq`;
CREATE TABLE `tb_lkcruokdfh`
(
    `col_hriruwciyw` blob(3794455272),
    CONSTRAINT UNIQUE INDEX `col_hriruwciyw` (`col_hriruwciyw`(11)),
