...
RENAME
TABLE `tb_wczdgujkxe` TO `tb_alxysrcrzk`, `tb_cvcsopmfuk` TO `tb_bcgwxxtrfr`;
RENAME
TABLE `tb_bcgwxxtrfr` TO `tb_qfvsxuvwcl`, `tb_alxysrcrzk` TO `tb_esuwpuxvcf`;
RENAME
TABLE `tb_qfvsxuvwcl` TO `tb_nsqdmubznz`, `tb_esuwpuxvcf` TO `tb_rmphoinujw`;
RENAME
TABLE `tb_nsqdmubznz` TO `tb_fgebibysbk`, `tb_rmphoinujw` TO `tb_xntcbmvmxc`;
RENAME
TABLE `tb_xntcbmvmxc` TO `tb_jmydpaxtpu`, `tb_fgebibysbk` TO `tb_bkldmexfby`;
CREATE TABLE `tb_wczdgujkxe`
(
    `col_mprviuzmsa` mediumint(215) unsigned zerofill,
    `col_wmnengkfur` decimal(6) NOT NULL,
