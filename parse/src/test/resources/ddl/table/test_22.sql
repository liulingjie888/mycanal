CREATE TABLE `tb_digeezyvpr`
(
    `col_obzlxjlpci` numeric(61) NULL,
    CONSTRAINT `symb_tszhfwqpyk` UNIQUE (`col_obzlxjlpci`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1;
CREATE TABLE `tb_mnebelpxsl`
(
    `col_iykolxhoko` numeric(31),
    `col_cgzreeozpp` enum('enum_or_set_0','enum_or_set_1','enum_or_set_2') CHARACTER SET utf8,
    `col_iraphtrlrd` varbinary(212) NOT NULL DEFAULT '\0',
    CONSTRAINT symb_gwsukjjxtg PRIMARY KEY (`col_iraphtrlrd`(31)),
    UNIQUE KEY `uk_rzlfipspah` (`col_cgzreeozpp`),
    UNIQUE `col_iraphtrlrd` (`col_iraphtrlrd`(25))
) DEFAULT CHARSET=utf8;
CREATE TABLE `tb_urqkphvawf` LIKE `tb_mnebelpxsl`;
RENAME
TABLE `tb_urqkphvawf` TO `tb_bafzppiguz`;
RENAME
TABLE `tb_digeezyvpr` TO `tb_sulmivgjpy`, `tb_bafzppiguz` TO `tb_fytbpxcdjl`;
RENAME
TABLE `tb_mnebelpxsl` TO `tb_ounmwpgnzl`, `tb_fytbpxcdjl` TO `tb_betlbqxtir`;
RENAME
TABLE `tb_betlbqxtir` TO `tb_qukciepnqi`;
RENAME
TABLE `tb_sulmivgjpy` TO `tb_mjsscuoztm`, `tb_ounmwpgnzl` TO `tb_xkdnjpdvmj`;
RENAME
TABLE `tb_xkdnjpdvmj` TO `tb_kaxlcttjna`, `tb_qukciepnqi` TO `tb_mjixwohckz`;
RENAME
TABLE `tb_mjsscuoztm` TO `tb_wdurcynllr`;
RENAME
TABLE `tb_kaxlcttjna` TO `tb_ayzakpuupb`;
CREATE TABLE `tb_digeezyvpr`
(
    `col_obzlxjlpci` numeric(61) NULL,
    CONSTRAINT `symb_tszhfwqpyk` UNIQUE (`col_obzlxjlpci`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1;
CREATE TABLE `tb_mnebelpxsl`
(
    `col_iykolxhoko` numeric(31),
    `col_cgzreeozpp` enum('enum_or_set_0','enum_or_set_1','enum_or_set_2') CHARACTER SET utf8,
    `col_iraphtrlrd` varbinary(212) NOT NULL DEFAULT '\0',
    CONSTRAINT symb_gwsukjjxtg PRIMARY KEY (`col_iraphtrlrd`(31)),
    UNIQUE KEY `uk_rzlfipspah` (`col_cgzreeozpp`),
    UNIQUE `col_iraphtrlrd` (`col_iraphtrlrd`(25))
) DEFAULT CHARSET=utf8;
CREATE TABLE `tb_urqkphvawf` LIKE `tb_mnebelpxsl`;
RENAME
TABLE `tb_urqkphvawf` TO `tb_bafzppiguz`;
RENAME
TABLE `tb_digeezyvpr` TO `tb_sulmivgjpy`, `tb_bafzppiguz` TO `tb_fytbpxcdjl`;
RENAME
TABLE `tb_mnebelpxsl` TO `tb_ounmwpgnzl`, `tb_fytbpxcdjl` TO `tb_betlbqxtir`;
RENAME
TABLE `tb_betlbqxtir` TO `tb_qukciepnqi`;
RENAME
TABLE `tb_sulmivgjpy` TO `tb_mjsscuoztm`, `tb_ounmwpgnzl` TO `tb_xkdnjpdvmj`;
RENAME
TABLE `tb_xkdnjpdvmj` TO `tb_kaxlcttjna`, `tb_qukciepnqi` TO `tb_mjixwohckz`;
RENAME
TABLE `tb_mjsscuoztm` TO `tb_wdurcynllr`;
RENAME
TABLE `tb_kaxlcttjna` TO `tb_ayzakpuupb`;
CREATE TABLE `tb_digeezyvpr`
(
    `col_obzlxjlpci` numeric(61) NULL,
    CONSTRAINT `symb_tszhfwqpyk` UNIQUE (`col_obzlxjlpci`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1;
CREATE TABLE `tb_mnebelpxsl`
(
    `col_iykolxhoko` numeric(31),
    `col_cgzreeozpp` enum('enum_or_set_0','enum_or_set_1','enum_or_set_2') CHARACTER SET utf8,
    `col_iraphtrlrd` varbinary(212) NOT NULL DEFAULT '\0',
    CONSTRAINT symb_gwsukjjxtg PRIMARY KEY (`col_iraphtrlrd`(31)),
    UNIQUE KEY `uk_rzlfipspah` (`col_cgzreeozpp`),
    UNIQUE `col_iraphtrlrd` (`col_iraphtrlrd`(25))
) DEFAULT CHARSET=utf8;
CREATE TABLE `tb_urqkphvawf` LIKE `tb_mnebelpxsl`;
RENAME
TABLE `tb_urqkphvawf` TO `tb_bafzppiguz`;
RENAME
TABLE `tb_digeezyvpr` TO `tb_sulmivgjpy`, `tb_bafzppiguz` TO `tb_fytbpxcdjl`;
RENAME
TABLE `tb_mnebelpxsl` TO `tb_ounmwpgnzl`, `tb_fytbpxcdjl` TO `tb_betlbqxtir`;
RENAME
TABLE `tb_betlbqxtir` TO `tb_qukciepnqi`;
RENAME
TABLE `tb_sulmivgjpy` TO `tb_mjsscuoztm`, `tb_ounmwpgnzl` TO `tb_xkdnjpdvmj`;
RENAME
TABLE `tb_xkdnjpdvmj` TO `tb_kaxlcttjna`, `tb_qukciepnqi` TO `tb_mjixwohckz`;
RENAME
TABLE `tb_mjsscuoztm` TO `tb_wdurcynllr`;
RENAME
TABLE `tb_kaxlcttjna` TO `tb_ayzakpuupb`;
