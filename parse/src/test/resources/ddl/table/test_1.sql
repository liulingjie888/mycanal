...
RENAME
TABLE `tb_qfmtewribz` TO `tb_qeokuaneue`;
RENAME
TABLE `tb_lttgkqunzl` TO `tb_uhjgsigdfr`, `tb_erkppdannn` TO `tb_xeoclzosgr`;
RENAME
TABLE `tb_quilrfojmp` TO `tb_hycsuwyoxr`, `tb_knsgjmsufx` TO `tb_bbvvzzczcw`;
RENAME
TABLE `tb_xeoclzosgr` TO `tb_fxxptceefr`;
RENAME
TABLE `tb_odnffrvnvl` TO `tb_rtnozmledi`;
RENAME
TABLE `tb_fxxptceefr` TO `tb_tekgceqrmv`;
RENAME
TABLE `tb_hycsuwyoxr` TO `tb_hcvvpyqbtd`, `tb_szlgabmztl` TO `tb_yfjonexaag`;
RENAME
TABLE `tb_tekgceqrmv` TO `tb_wewsxxqmxd`, `tb_uhjgsigdfr` TO `tb_mbvuditpbx`;
