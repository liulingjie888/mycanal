CREATE TABLE `tb_fzevsbnyxa`
(
    `col_wuvofglkav` blob,
    CONSTRAINT UNIQUE INDEX `col_wuvofglkav` (`col_wuvofglkav`(20)),
    UNIQUE INDEX `col_wuvofglkav_2` (`col_wuvofglkav`(18))
) DEFAULT CHARSET=latin1;
CREATE TABLE `tb_fdwsyimqus`
(
    `col_zqritfblef` set('enum_or_set_0','enum_or_set_1','enum_or_set_2') CHARACTER SET utf8mb4 NULL,
    `col_tvyzdewvuz` integer(17),
    `col_rigzrdrdlq` numeric(31, 15) NULL,
    CONSTRAINT UNIQUE INDEX `uk_ckjnyzlzhn` (`col_rigzrdrdlq`)
) DEFAULT CHARSET=latin1;
CREATE TABLE `tb_igmsbcjwnu`
(
    `col_sgxesfcvdh` blob,
    `col_tjhvpjnswu` bigint unsigned,
    `col_vvbowgfzum` varchar(200) CHARACTER SET utf8 DEFAULT ''
) ENGINE=InnoDB DEFAULT CHARSET=utf8;
RENAME
TABLE `tb_fdwsyimqus` TO `tb_ubrqfpxtmq`, `tb_fzevsbnyxa` TO `tb_wqdqkksoxc`;
RENAME
TABLE `tb_ubrqfpxtmq` TO `tb_aggxjxbfia`;
RENAME
TABLE `tb_aggxjxbfia` TO `tb_mszwzhbqqo`;
RENAME
TABLE `tb_wqdqkksoxc` TO `tb_osccbhgwdn`;
RENAME
TABLE `tb_mszwzhbqqo` TO `tb_caqffimhoa`;
RENAME
TABLE `tb_caqffimhoa` TO `tb_chzhfzbmgu`, `tb_igmsbcjwnu` TO `tb_bpukgjzglw`;
CREATE TABLE `tb_fzevsbnyxa`
(
    `col_wuvofglkav` blob,
    CONSTRAINT UNIQUE INDEX `col_wuvofglkav` (`col_wuvofglkav`(20)),
    UNIQUE INDEX `col_wuvofglkav_2` (`col_wuvofglkav`(18))
) DEFAULT CHARSET=latin1;
CREATE TABLE `tb_fdwsyimqus`
(
    `col_zqritfblef` set('enum_or_set_0','enum_or_set_1','enum_or_set_2') CHARACTER SET utf8mb4 NULL,
    `col_tvyzdewvuz` integer(17),
    `col_rigzrdrdlq` numeric(31, 15) NULL,
    CONSTRAINT UNIQUE INDEX `uk_ckjnyzlzhn` (`col_rigzrdrdlq`)
) DEFAULT CHARSET=latin1;
CREATE TABLE `tb_igmsbcjwnu`
(
    `col_sgxesfcvdh` blob,
    `col_tjhvpjnswu` bigint unsigned,
    `col_vvbowgfzum` varchar(200) CHARACTER SET utf8 DEFAULT ''
) ENGINE=InnoDB DEFAULT CHARSET=utf8;
RENAME
TABLE `tb_fdwsyimqus` TO `tb_ubrqfpxtmq`, `tb_fzevsbnyxa` TO `tb_wqdqkksoxc`;
RENAME
TABLE `tb_ubrqfpxtmq` TO `tb_aggxjxbfia`;
RENAME
TABLE `tb_aggxjxbfia` TO `tb_mszwzhbqqo`;
RENAME
TABLE `tb_wqdqkksoxc` TO `tb_osccbhgwdn`;
RENAME
TABLE `tb_mszwzhbqqo` TO `tb_caqffimhoa`;
RENAME
TABLE `tb_caqffimhoa` TO `tb_chzhfzbmgu`, `tb_igmsbcjwnu` TO `tb_bpukgjzglw`;
CREATE TABLE `tb_fzevsbnyxa`
(
    `col_wuvofglkav` blob,
    CONSTRAINT UNIQUE INDEX `col_wuvofglkav` (`col_wuvofglkav`(20)),
    UNIQUE INDEX `col_wuvofglkav_2` (`col_wuvofglkav`(18))
) DEFAULT CHARSET=latin1;
CREATE TABLE `tb_fdwsyimqus`
(
    `col_zqritfblef` set('enum_or_set_0','enum_or_set_1','enum_or_set_2') CHARACTER SET utf8mb4 NULL,
    `col_tvyzdewvuz` integer(17),
    `col_rigzrdrdlq` numeric(31, 15) NULL,
    CONSTRAINT UNIQUE INDEX `uk_ckjnyzlzhn` (`col_rigzrdrdlq`)
) DEFAULT CHARSET=latin1;
CREATE TABLE `tb_igmsbcjwnu`
(
    `col_sgxesfcvdh` blob,
    `col_tjhvpjnswu` bigint unsigned,
    `col_vvbowgfzum` varchar(200) CHARACTER SET utf8 DEFAULT ''
) ENGINE=InnoDB DEFAULT CHARSET=utf8;
RENAME
TABLE `tb_fdwsyimqus` TO `tb_ubrqfpxtmq`, `tb_fzevsbnyxa` TO `tb_wqdqkksoxc`;
RENAME
TABLE `tb_ubrqfpxtmq` TO `tb_aggxjxbfia`;
RENAME
TABLE `tb_aggxjxbfia` TO `tb_mszwzhbqqo`;
RENAME
TABLE `tb_wqdqkksoxc` TO `tb_osccbhgwdn`;
RENAME
TABLE `tb_mszwzhbqqo` TO `tb_caqffimhoa`;
RENAME
TABLE `tb_caqffimhoa` TO `tb_chzhfzbmgu`, `tb_igmsbcjwnu` TO `tb_bpukgjzglw`;
