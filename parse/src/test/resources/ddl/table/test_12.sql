...
RENAME
TABLE `tb_exsgxyoern` TO `tb_jjevlzpizx`;
RENAME
TABLE `tb_jjevlzpizx` TO `tb_bckwmwbhxe`;
RENAME
TABLE `tb_rexvlcurcc` TO `tb_reoiwwcvxn`, `tb_drxwrxamak` TO `tb_yewsyhywwt`;
RENAME
TABLE `tb_kjyszidrak` TO `tb_jaaspokopu`, `tb_reoiwwcvxn` TO `tb_zcarrcfbqk`;
RENAME
TABLE `tb_yewsyhywwt` TO `tb_qfbjzrlhat`, `tb_qhsljnukja` TO `tb_olxipktwgn`;
