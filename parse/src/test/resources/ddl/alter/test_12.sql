...
(
    ...
    `col_jqxasrchdv` double(240, 6
) NOT NULL,
  PRIMARY KEY (`col_jqxasrchdv`)
) DEFAULT CHARSET=utf8;
RENAME
TABLE `tb_xyumhtxajs` TO `tb_dwdawlyulx`, `tb_izihmqlvrc` TO `tb_tqqjgqvlty`;
RENAME
TABLE `tb_tqqjgqvlty` TO `tb_vltiykvgrd`;
RENAME
TABLE `tb_vltiykvgrd` TO `tb_fswsddbzby`;
...
ALTER TABLE `tb_jhqwojqbtj`
    ADD COLUMN `col_ksprsgkzrb` text(912789829) CHARACTER SET utf8mb4;
...
ALTER TABLE `tb_jhqwojqbtj`
    ADD `col_hemnlexcdd` int zerofill FIRST;
...
ALTER TABLE `tb_jhqwojqbtj`
    ADD PRIMARY KEY (`col_tbaacjfxyz`);
ALTER TABLE `tb_jhqwojqbtj`
    ALTER `col_egszzxhsvf` DROP DEFAULT;
