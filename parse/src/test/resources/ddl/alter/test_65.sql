...
RENAME
TABLE `tb_pdtvascuyy` TO `tb_rnradhkfon`, `tb_vsqjzkimwi` TO `tb_ambgylvzkp`;
...
ALTER TABLE `tb_ambgylvzkp`
    ADD COLUMN `col_xsgbuhbqwd` mediumblob AFTER `col_ebfnpjymeg`;
...
ALTER TABLE `tb_ambgylvzkp`
    ADD COLUMN `col_smbnzspzed` date NULL AFTER `col_savmueldgi`;
ALTER TABLE `tb_ambgylvzkp`
    ADD COLUMN `col_nzhjdxjqxl` blob AFTER `col_dkjrphxpni`;
...
ALTER TABLE `tb_ambgylvzkp`
    ADD PRIMARY KEY (`col_llpqomujks`, `col_gbegrurwao`);
ALTER TABLE `tb_ambgylvzkp`
    ADD UNIQUE (`col_gbegrurwao`, `col_ubwwasrhlw`(19));
ALTER TABLE `tb_ambgylvzkp`
    ADD UNIQUE KEY `uk_mvcfpncfde` (`col_xkicftbwbg`(17),`col_gbegrurwao`);
