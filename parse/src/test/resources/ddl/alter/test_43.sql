...
RENAME
TABLE `tb_imdtsfqnma` TO `tb_hbqbstsanm`;
RENAME
TABLE `tb_hbqbstsanm` TO `tb_uoquelzmdk`;
ALTER TABLE `tb_uoquelzmdk` ADD COLUMN (`col_bbvxosyuai` float, `col_wcdxzaykzl` smallint unsigned NULL DEFAULT '1');
ALTER TABLE `tb_uoquelzmdk`
    ADD `col_njhhehxbdq` mediumtext;
ALTER TABLE `tb_uoquelzmdk`
    ADD `col_znccfjlzyn` decimal(23, 10);
...
ALTER TABLE `tb_uoquelzmdk`
    ADD `col_btyauewuzi` mediumblob AFTER `col_bbvxosyuai`;
ALTER TABLE `tb_uoquelzmdk`
    ADD UNIQUE KEY `uk_sjpsswkher` (`col_bdbfyfsjoy`,`col_pheamargcl`);
ALTER TABLE `tb_uoquelzmdk`
    ALTER `col_lwcvaletgl` DROP DEFAULT;
ALTER TABLE `tb_uoquelzmdk`
    ALTER COLUMN `col_pheamargcl` DROP DEFAULT;
